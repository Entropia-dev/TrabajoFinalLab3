...
namespace Dao
{
    public class DaoCuenta
    {
        ...
        {
            String consulta = "Select * from Cuentas where Pass_Cue ='" + pro.get_contrasenia() + "'";
            return ds.existe(consulta);
        }
        ...
        /*
            ...
            */
        public int modificarCuenta(Cuentas pro)
        {

            // pro.get_email_cuenta(ds.ObtenerMaximo("SELECT max(idProducto) FROM Producto") + 1);
            SqlCommand comando = new SqlCommand();
            ArmarParametrosCuentamodificar(ref comando, pro);
            return ds.EjecutarProcedimientoAlmacenado(comando, "spmodificarcuenta");
        }
        private void ArmarParametrosCuentamodificar(ref SqlCommand Comando, Cuentas pro)
        {
